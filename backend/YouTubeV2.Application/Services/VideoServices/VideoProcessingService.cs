...
namespace YouTubeV2.Application.Services.VideoServices
{
    public class VideoProcessingService : BackgroundService, IVideoProcessingService
    {
        ...
        private async Task ConvertToMP4AndUploadVideoAsync(VideoProcessJob videoProcessJob, CancellationToken cancellationToken)
        {
            ...
            IVideoService videoService = serviceScope.ServiceProvider.GetRequiredService<IVideoService>();
            IBlobVideoService blobVideoService = serviceScope.ServiceProvider.GetRequiredService<IBlobVideoService>();
            IFileInspector fileInspector = serviceScope.ServiceProvider.GetRequiredService<IFileInspector>();
            ...
            try
            {
                ...
                {
                    using FileStream videoStream = fileInspector.OpenRead(videoProcessJob.Path);
                ...
                await using var outputFileStream = fileInspector.OpenRead(outputFilePath);
