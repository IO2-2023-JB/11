...
namespace YouTubeV2.Application.Services
{
    public class TicketService : ITicketService
    {
        ...
        {
            await CheckIfTargetExistsAsync(dto.TargetId, cancellationToken);

            var ticket = new Ticket(_dateTimeProvider.UtcNow, dto.TargetId, dto.Reason, submitter);
        ...
        public async Task<IEnumerable<GetTicketDto>> GetTicketListAsync(CancellationToken cancellationToken = default)
        {
            var tickets = await _context.Tickets
                ...
                .Select(x => new GetTicketDto(x))
                .ToListAsync(cancellationToken);

            return tickets;
        }

        private async Task CheckIfTargetExistsAsync(Guid id, CancellationToken cancellationToken)
        {
